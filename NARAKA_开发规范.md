...
- 禁止在业务代码中散落魔法数字。
- 配置源表是 `Config/Source/` 下的 UTF-8 BOM CSV，经本地配置编译器校验（ID唯一、外键存在、非负、堆叠上限、概率、品质、保底可兑现、等级连续）后才能生成。
- 客户端和服务端消费**同一份生成物**，数据模型只写一份并镜像；CI 的 `--check` 门禁保证两份副本一致。
- 服务端配置是经济判定的唯一权威；客户端配置**只用于展示**，不得用来推导任何账号资产变化。
- Unity 运行时不读 xlsx；云主机不安装 Excel、不运行配置编译。
- 不得手改生成的 JSON；改数值只能改源表并重新生成。
- 配置发布必须包含ConfigVersion、SHA-256清单、兼容范围和回滚版本。相同输入必须产生字节相同的输出。
- 编译器绝不输出密码、连接串或环境变量值。
...
- 迁移**只能新增**：不得修改已部署过的迁移文件，不得删除、重命名或覆写现有数据。
- 迁移必须兼容 MySQL 5.7.26：不得把 `CHECK` 当作约束（会被静默忽略），不得使用 `ADD COLUMN IF NOT EXISTS`；非负约束靠 `UNSIGNED` 列类型。
- 迁移器按分号拆分语句，因此**注释里不得出现分号**——一个杂正的分号会静默地把 `CREATE TABLE` 切成两半。每个迁移都要有语句数与契约测试。
- 关系型数据（好友、会话参与方）写入时必须在同一事务里完成全部相关行，不得留下单向关系。
...
- DeepSeek网页版每轮只能接收一个边界明确的页面、弹窗或组件任务；提示词必须写明现状、输入文件、输出文件、UI层级、手工挂载步骤、明确排除项和人工验收标准。完成一个模块并经Claude/Codex验收后，才能由Codex生成下一个模块的文字任务。
