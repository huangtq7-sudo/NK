...
- 真实`Socket.Select`运行时已接入Host，使用旧客户端的`127.0.0.1:8011`开发端点，并通过临时端口的握手、注册、登录、心跳闭环测试。
- 旧客户端实际300秒才发送心跳，旧服务端120秒判死会必然误断线。冻结期不改客户端传输实现，新适配边界将判死值设为360秒；传输层解冻后再统一为技术基线的30秒发送/120秒判死。
