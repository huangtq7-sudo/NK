...
- 完整P1云端切换尚未完成。2026-09-10首次使用`p1-lobby-systems-001`部署时，迁移`0001`–`0009`均执行成功，但迁移器最终验证SQL只列出21张表却要求计数27，并把9个版本错误写成8；部署脚本因此在移动Host目录前停止并恢复旧P1.1-A Host。该问题仅是验收清单错误，未修改既有迁移；修复后新增两项回归测试并通过服务端354/354。
- 云端NK库已经执行扩展式迁移`0003`–`0009`，部署前备份已完成；修复版迁移器仍会幂等重复执行并重新验证。真实P1 Host端点与客户端全功能冒烟等待`p1-lobby-systems-002`。
- 完整P1源代码已提交为`20e55112a4bdee2d6ae2f7c8ff35718de4964350`并推送到`origin/main`；迁移器验证修复等待独立提交与推送。
